...
Deploy `workers/subscription-lifecycle-cron` separately only after setting its real D1 database name/ID, setting `PTERODACTYL_PANEL_URL` as a Worker variable, binding `PTERODACTYL_APPLICATION_API_KEY` as a Worker secret, and testing it with Stripe test data. Its hourly Cron suspends only orders whose recorded grace period has expired.
